            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-description">
                        <a class="btn btn-success" href="{{ route('panel.products.create') }}">Add</a>
...
{{--                                        </label>--}}
                                    ...
                                    <td class="d-flex">
                                        <a class="btn btn-primary mr-3"  href="{{ route('panel.products.edit',$model->id) }}">Edit</a>
    ...
    <script>
        $(document).ready(function(){
            $('.status').on('change', function(){
                ...
                // Send AJAX request
                $.ajax({
                    url: '{{ route('panel.products.status') }}',
                    method: 'POST',
                    data: {
                        itemId: itemId,
            ...
            $('.deleteBtn').on('click', function(){
                ...
                alertify.confirm('Warning',"Are you sure for delete?",
                    function(){
                        $.ajax({
                            url: '{{ route('panel.products.destroy') }}',
                            method: 'DELETE',
                            data: {
                                id: id,
                                _token: '{{ csrf_token() }}'
